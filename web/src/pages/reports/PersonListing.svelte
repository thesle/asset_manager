  ...
  async function toggleExpand(personId) {
    ...
    if (!personAssets[personId]) {
      ...
      try {
        ...
        const assetsWithProps = await Promise.all(assets.map(async (assignment) => {
          try {
...
<Card>
  ...
  {:else if persons.length === 0}
    <p class="has-text-grey">No persons found</p>
  {:else}
        ...
        <div class="accordion-item" class:is-expanded={expandedPersonId === person.ID}>
          ...
          >
            <div class="accordion-icon">
              <span class="icon">
                <i class="fas" class:fa-chevron-down={expandedPersonId === person.ID} class:fa-chevron-right={expandedPersonId !== person.ID}></i>
              </span>
            </div>
                    ...
                    <tbody>
                      {#each personAssets[person.ID] as assignment}
                        <tr>
                          <td>
                            <a 
                              href="#/assets?edit={assignment.AssetID}" 
                              class="has-text-link"
                            >
