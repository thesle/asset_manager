<script>
  ...
  async function loadData() {
    loading = true;
    try {
      const result = await api.getAttributes();
      items = result || [];
    } catch (err) {
      notifications.error('Failed to load attributes');
    } finally {
      loading = false;
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        await api.updateAttribute(editing.ID, data);
        notifications.success('Attribute updated');
      } else {
        await api.createAttribute(data);
        notifications.success('Attribute created');
      }
      showModal = false;
      await loadData();
    } catch (err) {
  ...
  async function handleDelete() {
    try {
      ...
      notifications.success('Attribute deleted');
...
<p class="subtitle">Custom attributes that can be attached to persons (e.g., Team, State, Active Directory)</p>

<Card>
  <DataTable {columns} data={items} {loading} emptyMessage="No attributes found" />
</Card>

<Modal bind:active={showModal} title={editing ? 'Edit Attribute' : 'New Attribute'} size="small">
